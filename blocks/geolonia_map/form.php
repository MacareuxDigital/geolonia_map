...
<div class="ccm-tab-content" id="ccm-tab-content-geolonia-map-basics">
    <fieldset>
        <div class="form-group">
            <?= $form->label('api_key', t('API Key')) ?>
            <?= $form->text('api_key', $api_key, ['placeholder' => 'YOUR-API-KEY']) ?>
            <div id="api_key_alert" class="help-block"><?= t('You can get an API Key from %shere%s.', '<a href="https://app.geolonia.com/#/api-keys" target="_blank">', '</a>') ?></div>
        </div>
        <div class="form-group">
...
<div class="ccm-tab-content" id="ccm-tab-content-geolonia-map-advanced">
    <fieldset>
        <div class="form-group">
...
<div class="ccm-tab-content" id="ccm-tab-content-geolonia-map-preview">
    <div class="form-group">
        <div id="preview-geolonia-map-alert" class="alert alert-warning hidden" role="alert"><?= t('You must input API Key.') ?></div>
        <div id="preview-geolonia-map-error" class="alert alert-danger hidden" role="alert"></div>
        <div id="preview-geolonia-map-container"></div>
    </div>
</div>
<style>
    #preview-geolonia-map-container > div {height: 300px;}
</style>
<script>
            ...
            if (elemApiKey.value) {
                ...
                script.onload = function () {
                    let elemMap = document.createElement('div'),
                        markerContentEditor = document.querySelector('#ccm-tab-content-geolonia-map-advanced .cke_editable')
                    ...
                    elemMapError.classList.add('hidden')
                    try {
                        new geolonia.Map('#preview-geolonia-map')
                    } catch (error) {
                        elemMapError.classList.remove('hidden')
                        elemMapError.innerText = error
                    }
                }
                document.getElementsByTagName('head')[0].appendChild(script)
                elemMapAlert.classList.add('hidden')
            } else {
                elemMapAlert.classList.remove('hidden')
            }
        }
    elemApiKey.addEventListener('focusout', () => {
        if (elemApiKey.value) {
            elemApiKeyAlert.classList.add('hidden')
        } else {
            elemApiKeyAlert.classList.remove('hidden')
        }
    })
    ...
    document.querySelector('[data-tab=geolonia-map-preview]').addEventListener('click', () => {
        loadMap()
    })
    if (elemApiKey.value) {
        elemApiKeyAlert.classList.add('hidden')
